...
module Puavo
  module Integrations
    ...
    def self.do_operation(request_id, url, operation, system, parameters)
      ...
      $rest_flog.info(nil, "[#{request_id}] do_operation(): sending network request")

      response = http.request(request)

      $rest_flog.info(nil, "[#{request_id}] do_operation(): network request complete")
      ...
      $rest_flog.info(nil, "[#{request_id}]   Raw response code: \"#{code}\"")
      $rest_flog.info(nil, "[#{request_id}]   Raw response body: \"#{body}\"")
      ...
      unless reply_data.include?('version') && reply_data.include?('operation') &&
        ...
        $rest_flog.error(nil, "[#{request_id}] Incomplete reply, not all required fields are present")
        raise OperationError, 'malformed_reply'
      end

      $rest_flog.info(nil, "[#{request_id}] Received a reply with ID \"#{reply_data['reply_id']}\"")
      $rest_flog.info(nil, "[#{request_id}] Request roundtrip time: #{now - reply_data['request_timestamp']} seconds")

      if reply_data['request_id'] != request_id
        $rest_flog.error(nil, "[#{request_id}] Reply request ID (#{reply_data['request_id']}) is not the one we sent!")
        raise OperationError, 'malformed_reply'
      end

      if reply_data['version'] != 0
        $rest_flog.error(nil, "[#{request_id}] Invalid reply version (#{reply_data['version']})")
      ...
      unless SYSTEM_REPLY_CODES.include?(code) || OPERATION_REPLY_CODES.include?(code)
        $rest_flog.error(nil, "[#{request_id}] Unknown reply code \"#{code}\"")
        raise OperationError, 'malformed_reply'
      end
    ...
    def self.azure_change_password(request_id, params, args)
      ...
      unless params.include?('url')
        $rest_flog.error(nil, "[#{request_id}] azure_change_password(): missing server URL in 'params'")
        raise OperationError, 'configuration_error'
      end

      unless args.include?(:organisation) && args.include?(:user) && args.include?(:new_password)
        $rest_flog.error(nil, "[#{request_id}] azure_change_password(): missing required arguments")
        raise OperationError, 'configuration_error'
      end

      url = params['url']

      $rest_flog.info(nil, "[#{request_id}] azure_change_password(): sending a request to \"#{url}\"")
      ...
      if response[:code] == 'user_not_found'
        # whatever
        $rest_flog.info(nil, "[#{request_id}] azure_change_password(): ignoring a missing user, assuming everything is OK")
        return true, nil
      end
    ...
    def self.google_change_password(request_id, params, args)
      # verify our parameters
      unless params.include?('url')
        $rest_flog.error(nil, "[#{request_id}] google_change_password(): missing server URL in 'params'")
        raise OperationError, 'configuration_error'
      end

      unless args.include?(:organisation) && args.include?(:user) && args.include?(:new_password)
        $rest_flog.error(nil, "[#{request_id}] google_change_password(): missing required arguments")
        raise OperationError, 'configuration_error'
      end

      url = params['url']

      $rest_flog.info(nil, "[#{request_id}] google_change_password(): sending a request to \"#{url}\"")
      ...
      if response[:code] == 'user_not_found'
        # whatever
        $rest_flog.info(nil, "[#{request_id}] google_change_password(): ignoring a missing user, assuming everything is OK")
        return true, nil
      end
    ...
    def self.do_synchronous_action(action, system, request_id, params, **args)
      ...
      $rest_flog.info(nil, "[#{request_id}] do_synchronous_action(): starting action")

      begin
        status, message = send(SYNCHRONOUS_ACTIONS[action][system], request_id, params, args)
        return status, message
      rescue Errno::ECONNREFUSED => e
        $rest_flog.error(nil, "[#{request_id}] do_synchronous_action(): caught ECONNREFUSED: #{e}")
        return false, 'network_error'
      rescue OperationError => e
        $rest_flog.error(nil, "[#{request_id}] do_synchronous_action(): caught OperationError: #{e}")
        return false, e.to_s
      rescue StandardError => e
        $rest_flog.error(nil, "[#{request_id}] do_synchronous_action(): caught StandardError: #{e}")

        if e.to_s == 'execution expired'
          $rest_flog.error(nil, "[#{request_id}]   -> this is a network timeout, can't contact the synchronisation server")
          return false, 'network_error'
        end
