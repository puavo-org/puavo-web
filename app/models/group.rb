...
class Group < BaseGroup
  ...
  ldap_mapping dn_attribute: 'puavoId',
               prefix: 'ou=Groups',
               classes: %w[top posixGroup puavoEduGroup sambaGroupMapping]
  ...
  belongs_to :school, class_name: 'School', foreign_key: 'puavoSchool', primary_key: 'dn'
  ...
  # NOTE: add_user() and remove_user() both have to manipulate two lists independently.
  # LDAP has no transactions and sometimes only one of the lists gets updated. So we
  # need to suppress some exceptions if we don't want the code to abort half-way.
  # You can pass multiple operations to ldap_modify_operation() and they *should* be
  # atomic, but I don't trust it.
  ...
  def member?(user_id)
    self.members.map(&:puavoId).include?(user_id.to_i)
  end
end
