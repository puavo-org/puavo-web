...
class School < BaseGroup
  ...
  ldap_mapping dn_attribute: 'puavoId',
               prefix: 'ou=Groups',
               classes: %w[top posixGroup puavoSchool sambaGroupMapping]

  has_many :members, class_name: 'User', primary_key: 'dn', foreign_key: 'puavoSchool'
  has_many :user_members, class_name: 'User', wrap: 'member', primary_key: 'dn'
  has_many :user_school_admins, class_name: 'User', primary_key: 'dn', foreign_key: 'puavoAdminOfSchool'
  has_many :user_member_uids, class_name: 'User', wrap: 'memberUid', primary_key: 'uid'
  has_many :groups, class_name: 'Group', primary_key: 'dn', foreign_key: 'puavoSchool'

  attr_accessor :image

  before_save :set_puavo_mountpoint, :set_preferred_language

  validate :validate_school_code, :validate_group_name, :validate_name_prefix, :validate_name,
           :validate_puavoconf, :validate_wlan_attributes, :validate_image

  alias v1_as_json as_json
  ...
  def remove_user(user)
    begin
      self.ldap_modify_operation(:delete, [{ 'memberUid' => [user.uid] }, { 'member' => [user.dn.to_s] }])
    rescue ActiveLdap::LdapError::NoSuchAttribute
      logger.warn "Cannot remove nonexistent memberUid=#{user.uid} or member=#{user.dn} from #{displayName}"
    end

    if Array(user.puavoAdminOfSchool).map(&:to_s).include?(self.dn.to_s)
      begin
        self.ldap_modify_operation(:delete, [{ 'puavoSchoolAdmin' => [user.dn.to_s] }])
      rescue ActiveLdap::LdapError::NoSuchAttribute
        logger.warn "Cannot remove nonexistent puavoSchoolAdmin=#{user.dn} from #{displayName}"
      end
    end
  end
  ...
  def printers
    servers_dn = Server.find(:all, { attribute: 'puavoSchool', value: self.dn })
                       .map(&:dn)
  ...
  def boot_servers
    Server.find(:all, { attribute: 'puavoSchool', value: self.dn.to_s })
          .select { |s| s.puavoDeviceType == 'bootserver' }
  end
  ...
  def add_wireless_printer(printer)
    ...
    begin
      ldap_modify_operation(:add, [{ 'puavoWirelessPrinterQueue' => printer }])
    rescue ActiveLdap::LdapError::TypeOrValueExists
    end
  end
  ...
  def remove_wireless_printer(printer)
    printer = self.class.ensure_dn(printer)
    begin
      ldap_modify_operation(:delete, [{ 'puavoWirelessPrinterQueue' => [printer.to_s] }])
    rescue ActiveLdap::LdapError::NoSuchAttribute
    end
  end
end
