...
class Printer < LdapBase
  ldap_mapping dn_attribute: 'puavoId',
               prefix: 'ou=Printers',
               classes: %w[top cupsPrinter puavoPrinterQueue]

  before_validation :set_puavo_id, :set_empty_location

  def validate
    Printer.find(:all, attribute: 'printerDescription', value: self.printerDescription).each do |printer|
  ...
  def set_empty_location
    self.printerLocation = '-' if self.printerLocation.to_s.empty?
  end
end
